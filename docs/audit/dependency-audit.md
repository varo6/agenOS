...
| `whisper-server`, `whisper-server-baseline` | `components/stt/paths.ts`, `components/stt/engine.ts`; `scripts/agenos-whisper-server` | build propio de whisper.cpp | No | Garantizado por `scripts/build-ui.sh`. Motor residente único para Electron y para `/api/speech/*`; escucha solo en `127.0.0.1`. |
...
| `ggml-base-q5_1.bin`, `ggml-silero-v5.1.2.bin` | `components/stt/paths.ts` vía el manifiesto `stt.env` | descarga verificada por SHA1 en `scripts/build-ui.sh` | No | Ambos viajan en la ISO: el STT funciona sin Internet. El runtime lee del manifiesto el nombre exacto que declaró el build. |
