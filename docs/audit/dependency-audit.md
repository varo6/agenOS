...
| `whisper-cli`, `whisper-cli-baseline` | `components/ui/src/electron/main.ts:314-408`; `components/installer-ui/src/bun/speech/stt.ts:54-61,211-321` | build propio de whisper.cpp | No | Garantizado por `scripts/build-ui.sh`, que compila ambos binarios y empaqueta el modelo `ggml-small.bin` en `/opt/agenos/system/whisper.cpp`. |
