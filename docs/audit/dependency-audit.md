...
| `arecord` | `components/stt/paths.ts` (candidatos), `components/stt/capture.ts` | `alsa-utils` | Sí: `base` | Garantizado. Graba PCM crudo sin duración fija; quien corta es el VAD. |
| `voxtype` | `components/stt/voxtype-engine.ts` | build desde el tag fijado de Voxtype | No | Motor predeterminado. Se precarga durante la captura, devuelve JSON por stdin/stdout y sale después de cada frase. La licencia MIT viaja como `LICENSE.voxtype`. |
| `whisper-server`, `whisper-server-baseline` | `components/stt/paths.ts`, `components/stt/engine.ts`; `scripts/agenos-whisper-server` | build propio de whisper.cpp | No | Fallback con `AGENOS_STT_ENGINE=whisper.cpp`. Ya no arranca durante el boot. |
| `agenos-vad-capture`, `agenos-vad-capture-baseline` | `components/stt/capture.ts` | fuente propio en `tools/whisper-vad-capture`, compilado dentro del árbol de whisper.cpp | No | Garantizado por `scripts/build-ui.sh`. Corta la grabación con Silero VAD. |
| `ggml-small-q5_1.bin`, `ggml-silero-v5.1.2.bin` | `components/stt/paths.ts` vía el manifiesto `stt.env` | descarga verificada por SHA1 en `scripts/build-ui.sh` | No | Ambos viajan en la ISO: el STT funciona sin Internet. El runtime lee del manifiesto el nombre exacto que declaró el build. |
| `whisper-cli`, `whisper-cli-baseline` | ya no lo usa el runtime; se sigue empaquetando para diagnóstico manual | build propio de whisper.cpp | No | Garantizado por `scripts/build-ui.sh`. |
| `ffmpeg` | `components/stt/paths.ts` (candidatos); `components/installer-ui/src/bun/speech/stt.ts` | `ffmpeg` | Sí: `base` | Garantizado. Solo lo necesita la ruta HTTP, para pasar el webm/ogg del navegador a 16 kHz mono. |
