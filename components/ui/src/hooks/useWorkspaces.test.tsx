...
describe("resolveWorkspaceSubscription", () => {
  test("sin transporte devuelve null y la interfaz sigue con el sondeo", () => {
    expect(resolveWorkspaceSubscription(undefined)).toBe(null);
    expect(resolveWorkspaceSubscription({})).toBe(null);
  });

  test("reenvía los escritorios válidos del stream y descarta los que no existen", () => {
    type StateListener = (state: { activeWorkspace?: unknown }) => void;
    let pushed: StateListener | null = null;
    const unsubscribe = vi.fn();
    const client = {
      subscribeWorkspaceChanges: (listener: StateListener) => {
        pushed = listener;
        return unsubscribe;
      },
    };

    const subscription = resolveWorkspaceSubscription(client);
    ...
    pushed!({ activeWorkspace: 3 });
    pushed!({ activeWorkspace: 9 });
    pushed!({ activeWorkspace: "2" });
    pushed!({});
